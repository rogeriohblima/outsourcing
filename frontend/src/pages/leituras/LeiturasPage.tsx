 ...
 *  1. Lançamento manual: formulário com contador, data e observação
 *  2. Leitura SNMP automática: dispara leitura no endpoint /leituras/snmp
 *
 * Ambas exigem contrato_id (vincula a leitura ao contrato para cálculo de franquia).
 * Filtros: período (mês/ano) e contrato.
 */
...
import { z } from 'zod'
import { ClipboardEdit, Trash2, Zap } from 'lucide-react'
import { contratosApi, impressorasApi, leiturasApi, tiposImpressaoApi } from '@/api/endpoints'
import {
  AlertMessage, ConfirmDialog, EmptyState, FormField,
  Modal, PageHeader, PageSpinner, Spinner, formatDate,
...
const camposComuns = {
  ...
  impressora_num_serie: z.string().min(1, 'Selecione a impressora'),
...
const manualSchema = z.object({
  ...camposComuns,
  contador:   z.coerce.number().min(0, 'Informe o contador'),
  data:       z.string().min(1, 'Informe a data'),
  observacao: z.string().optional(),
})

const snmpSchema = z.object(camposComuns)
...
function CamposComuns({ register, errors }: { register: any; errors: any }) {
  ...
  const nomeImpressora = (i: { num_serie: string; modelo?: { fabricante: string; modelo: string } | null; local: { setor: string } }) => {
    const modelo = i.modelo ? `${i.modelo.fabricante} ${i.modelo.modelo}` : i.num_serie
    return `${modelo} (${i.local.setor})`
  }

  return (
    <>
      <FormField label="Contrato" error={errors.contrato_id?.message} required>
        <select {...register('contrato_id')} className={`input ${errors.contrato_id ? 'input-error' : ''}`}>
      ...
      <FormField label="Impressora" error={errors.impressora_num_serie?.message} required>
        <select {...register('impressora_num_serie')} className={`input ${errors.impressora_num_serie ? 'input-error' : ''}`}>
          <option value="">Selecione…</option>
          {impressoras?.map((i) => (
            <option key={i.num_serie} value={i.num_serie}>{nomeImpressora(i)}</option>
          ))}
        </select>
      </FormField>
...
function ManualForm({ onSubmit, isLoading }: { onSubmit: (d: ManualFormData) => void; isLoading: boolean }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <CamposComuns register={register} errors={errors} />
      <div className="grid grid-cols-2 gap-4">
        <FormField label="Contador (total acumulado)" error={errors.contador?.message} required>
          <input {...register('contador')} type="number" min={0}
            className={`input ${errors.contador ? 'input-error' : ''}`}
            placeholder="98765" />
        </FormField>
      ...
      <FormField label="Observação (opcional)" error={errors.observacao?.message}>
        <textarea {...register('observacao')} rows={2} className="input"
          placeholder="SNMP indisponível — leitura manual no painel…" />
      </FormField>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-700">
        O sistema lerá o contador diretamente na impressora via SNMP.
      ...
      <div className="flex justify-end pt-2">
        <button type="submit" disabled={isLoading} className="btn-primary">
          {isLoading
            ? <><Spinner className="h-4 w-4" /> Lendo via SNMP…</>
            : <><Zap className="w-4 h-4" /> Ler via SNMP</>
          }
        </button>
      </div>
    </form>
...
function nomeImpressoraLeitura(leitura: LeituraOut): string {
  if (leitura.impressora?.modelo) {
    return `${leitura.impressora.modelo.fabricante} ${leitura.impressora.modelo.modelo}`
  }
  return leitura.impressora_num_serie
}
...
export default function LeiturasPage() {
  ...
  const manualM = useMutation({
    ...
    onSuccess: (l) => {
      ...
      setShowManual(false)
      setSuccessMsg(`Leitura registrada — contador: ${l.contador.toLocaleString('pt-BR')}`)
    },
  ...
  return (
    <div>
      <PageHeader
        title="Leituras de Contadores"
        subtitle="Registros mensais de contadores das impressoras"
        actions={
      ...
      <div className="card mb-4 flex flex-wrap gap-3 items-center">
        ...
        <input type="number" value={filterAno}
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th className="text-right">Contador</th>
              <th>Data</th>
              <th>Origem</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {(leituras ?? []).length === 0
              ? <tr><td colSpan={7}><EmptyState message="Nenhuma leitura para este período." /></td></tr>
              : (leituras ?? []).map((l) => (
                <tr key={l.id}>
                  <td>
                    <div className="font-medium text-sm">{nomeImpressoraLeitura(l)}</div>
                    <div className="text-xs text-gray-400 font-mono">{l.impressora_num_serie}</div>
                  </td>
                  <td className="text-gray-600">{l.impressora?.local?.setor ?? '—'}</td>
                  <td className="text-xs">{l.tipo_impressao?.descricao ?? '—'}</td>
                  <td className="text-right font-mono font-semibold text-fab-700">
                    {l.contador.toLocaleString('pt-BR')}
                  </td>
                  ...
                  <td className="text-right">
                    <button onClick={() => setDeleteItem(l)}
                      className="btn btn-danger btn-sm" title="Excluir">
      ...
      <Modal isOpen={showManual} onClose={() => setShowManual(false)}
        title="Lançamento Manual de Leitura" size="lg">
        <ManualForm onSubmit={(d) => manualM.mutate(d)} isLoading={manualM.isPending} />
      </Modal>

      <Modal isOpen={showSNMP} onClose={() => setShowSNMP(false)}
        title="Leitura Automática via SNMP" size="lg">
      ...
      <ConfirmDialog
        ...
        message={deleteItem ? `Excluir a leitura do contador ${deleteItem.contador.toLocaleString('pt-BR')} de "${nomeImpressoraLeitura(deleteItem)}"?` : ''}
